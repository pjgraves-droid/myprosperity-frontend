  ...
  return (
    <>
      <PageHeader
        ...
        action={<button className="rounded-md bg-emerald-600 text-white text-sm px-4 py-2 hover:bg-emerald-700">Upload</button>}
      />
      <Card>
              ...
              <tr key={d.ID}>
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-3">
                    <span className="grid place-items-center w-9 h-9 rounded-lg bg-slate-100 text-slate-500">
                      <FileText size={18} />
                    </span>
                    <div>
                      <div className="font-medium">{d.Description}</div>
                      <div className="text-xs text-slate-500">{d.File.FileName}</div>
                    </div>
                  </div>
                </td>
                <td className="py-3 pr-4 text-slate-600">{d.CreatedBy}</td>
                <td className="py-3 pr-4 text-slate-500 whitespace-nowrap">{shortDate(d.LastUpdated)}</td>
                <td className="py-3 pr-4 text-slate-500 tabular-nums">{fmtSize(d.File.SizeBytes)}</td>
                <td className="py-3 pr-4">
                  <Badge tone={s.tone}>{s.label}</Badge>
                </td>
                <td className="py-3 text-right">
                  {d.DocumentSigningStatus === DocumentSigningStatus.Pending ? (
                    <button className="text-sm font-medium text-emerald-700 hover:underline">Sign</button>
                  ) : (
                    <button className="text-sm text-slate-500 hover:underline">Download</button>
                  )}
                </td>
              </tr>
            );
